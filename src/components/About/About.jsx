...
export default function About() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.to(watchRef.current, { y: -8, duration: 3, repeat: -1, yoyo: true, ease: 'sine.inOut', delay: 1 })
      ...
      specRefs.current.forEach((spec, i) => {
        ...
        gsap.fromTo(spec,
          ...
          { y: 0, opacity: 1, filter: 'blur(0px)', duration: 0.6, ease: 'power3.out',
            scrollTrigger: { trigger: sectionRef.current, start: 'top 65%' }, delay: 0.2 + i * 0.1 }
        )
      })

      gsap.to(bgGlowRef.current, {
        scale: 1.1, opacity: 0.5, repeat: -1, yoyo: true, duration: 5, ease: 'sine.inOut',
      })
    }, sectionRef)
    ...
    <section id="about" ref={sectionRef} className={styles.section}>
      ...
      <div className={styles.watchWrap} ref={watchRef}>
        <div ref={watchInnerRef} className={styles.watchInner}>
          <OptimizedImage path="about-watch.jpg" alt="REISER watch" width={1024} height={1024} className={styles.watchImage} />
        </div>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.specs}>
          {specs.map((s, i) => (
            <div key={s.value} ref={(el) => { specRefs.current[i] = el }} className={styles.spec}>
