...
export default function Collection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(headingRef.current, { y: 40, opacity: 0, filter: 'blur(6px)' }, { y: 0, opacity: 1, filter: 'blur(0px)', duration: 0.7, ease: 'power3.out' })
      cardsRef.current.forEach((card, i) => {
        if (!card) return
        tl.fromTo(card, { y: 80, opacity: 0, scale: 0.9, rotateY: i === 0 ? -15 : i === 2 ? 15 : 0 }, { y: 0, opacity: 1, scale: 1, rotateY: 0, duration: 0.9, ease: 'power4.out' }, `-=${i === 0 ? 0.4 : 0.5}`)
      })
  ...
  return (
    <section id="collection" ref={sectionRef} className={styles.section}>
      ...
      <div className={styles.carousel}>
        {variants.map((v, i) => (
          <div key={v.name} ref={(el) => { cardsRef.current[i] = el }} className={`${styles.card} ${i === activeIdx ? styles.cardActive : ''}`} onMouseEnter={() => setActiveIdx(i)}>
            <div className={styles.cardInner}>
              <div className={styles.imageWrap}>
                <OptimizedImage path="about-watch.jpg" alt={v.name} width={1024} height={1024} className={styles.cardImage} style={{ filter: v.filter }} draggable={false} />
      ...
      <div className={styles.indicators}>
        {variants.map((_, i) => (
          <span key={i} className={`${styles.dot} ${i === activeIdx ? styles.dotActive : ''}`} onMouseEnter={() => setActiveIdx(i)} />
        ))}
      </div>
    </section>
  )
