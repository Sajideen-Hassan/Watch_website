...
const features = [
  {
    ...
    image: '/images/feature-precision.jpg',
    pos: '50% 30%',
  },
  {
    num: '02',
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      panels.current.forEach((panel, i) => {
        ...
        tl.fromTo(nums.current[i],
          { y: 60, opacity: 0, scale: 0.8 },
          { y: 0, opacity: 1, scale: 1, duration: 0.5, ease: 'power3.out' }
        )

        tl.fromTo(imageWraps.current[i],
          { x: i % 2 === 0 ? -80 : 80, opacity: 0, scale: 1.1 },
          { x: 0, opacity: 1, scale: 1, duration: 0.9, ease: 'power4.out' },
          '-=0.3'
        )

        tl.fromTo(titles.current[i],
          { clipPath: 'inset(0 100% 0 0)' },
          { clipPath: 'inset(0 0% 0 0)', duration: 0.6, ease: 'power3.out' },
          '-=0.5'
        )

        tl.fromTo(descs.current[i],
    ...
    <section id="features" ref={sectionRef} className={styles.section}>
      {features.map((f, i) => (
        <div
          ...
          className={`${styles.panel} ${i % 2 !== 0 ? styles.panelReverse : ''}`}
        >
          <div
            ref={(el) => { imageWraps.current[i] = el }}
          ...
          <div className={styles.textSide}>
            <span ref={(el) => { nums.current[i] = el }} className={styles.num}>
              {f.num}
            </span>
            <h3
            ...
            <p ref={(el) => { descs.current[i] = el }} className={styles.desc}>
              {f.desc}
            </p>
          </div>
        </div>
